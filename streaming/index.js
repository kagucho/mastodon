    ...
    const listener = message => {
      ...
      if (needsFiltering && event === 'update') {
        pgPool.connect((err, client, done) => {
          ...
          client.query(`SELECT target_account_id FROM account_relations WHERE account_id = $1 AND target_account_id IN (${placeholders(targetAccountIds, 1)}) AND type IN ('block', 'mute')`, [req.accountId].concat(targetAccountIds), (err, result) => {
            done()
