  ...
  describe '#push_bulk' do
    ...
    context 'when status is reblog' do
      ...
      context 'when timeline type is home' do
        it 'pushes to subscribed timelines' do
          Redis.current.set("subscribed:timeline:#{account.id}", '1')
          expect_to_publish(account, reblog)
          FeedManager.instance.push_bulk(:home, [account], status)
          expect(Redis.current.zscore("feed:home:#{account.id}", reblog.id)).to eq status.id
        end

        it 'does not push to unsubscribed timelines' do
          expect_not_to_publish
          FeedManager.instance.push_bulk(:home, [account], status)
          expect(Redis.current.zscore("feed:home:#{account.id}", reblog.id)).to eq nil
        end
      end
      ...
      it 'does not reinsert if the original status is within 40 statuses from top' do
        Redis.current.zadd("feed:type:#{account.id}", status.id, reblog.id)
        expect_not_to_publish
        FeedManager.instance.push_bulk('type', [account], status)
      end
    end
    ...
    context 'when status is not reblog' do
      ...
      context 'when timeline type is home' do
        it 'pushes to subscribed timelines' do
          ...
          FeedManager.instance.push_bulk(:home, [account], status)
          expect(Redis.current.zscore("feed:home:#{account.id}", status.id)).to eq status.id
        end

        it 'does not push to unsubscribed timelines' do
          expect_not_to_publish
          FeedManager.instance.push_bulk(:home, [account], status)
          expect(Redis.current.zscore("feed:home:#{account.id}", status.id)).to eq nil
        end
      end
