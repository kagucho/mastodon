...
class FeedManager
  ...
  def push_bulk(timeline_type, accounts, status)
    account_ids = subscribing_account_ids(timeline_type, accounts)
    ...
    if status.reblog?
      ...
      filtered_account_ranks = account_ranks.lazy.reject do |account_rank|
      ...
      filtered_account_ids = filtered_account_ranks.map { |account_rank| account_rank[:id] }

      redis.pipelined do
        Account.where(id: filtered_account_ids.to_a).each do |account|
          publish(key(timeline_type, account.id), account, status, status.reblog_of_id)
        end
      end
    else
      timeline_keys = nil
      redis.pipelined do
        timeline_keys = Account.where(id: account_ids.to_a).map do |account|
          timeline_key = key(timeline_type, account.id)
          publish(timeline_key, account, status, status.id)
          timeline_key
        end
      end
  ...
  def publish(timeline_key, account, status, member)
    redis.zadd(timeline_key, status.id, member)

    message = InlineRenderer.render(status, account, 'api/v1/statuses/show')
    redis.publish("timeline:#{account.id}", Oj.dump(event: :update, payload: message, queued_at: (Time.now.to_f * 1000.0).to_i))
  end

  def subscribing_account_ids(timeline_type, accounts)
  ...
  def subscribed_bulk(account_ids)
    redis.pipelined { account_ids.each(&method(:subscribed)) }
  end

  def subscribed(account_id)
    redis.get("subscribed:timeline:#{account_id}")
  end
